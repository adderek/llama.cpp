...
// TQ weight types have no mmvq/mmq kernels; they use the fused mul_mat_vec_tq path or dequant + cuBLAS.
static bool ggml_cuda_is_tq_weight_type(ggml_type type) {
    return type == GGML_TYPE_TQ4_1S || type == GGML_TYPE_TQ3_1S;
}
...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (ggml_cuda_is_tq_weight_type(src0->type)) {
        if (ne11 == 1) {
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const bool is_tq_weight_id = ggml_cuda_is_tq_weight_type(src0->type);
    if (src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
